...
class TemporalAwaretAPE(nn.Module):
    """
    Combines tAPE's frequency correction with temporal offset encoding.
    Computes frequency term dynamically based on actual sequence length.
    """
    ...
    def forward(self, x, x_dates):
        """
        Args:
            x: [batch_size, seq_len, d_model]
            x_dates: [batch_size, seq_len] - temporal offsets
        """
        ...
        # incorporate sequence length
        omega_new_k = omega_k * (d_model / seq_len)        
        position = x_dates.unsqueeze(-1).float()
        
        pe = torch.zeros_like(x)
        pe[:, :, 0::2] = torch.sin(position * omega_new_k)
        pe[:, :, 1::2] = torch.cos(position * omega_new_k)
        
        x = x + self.scale_factor * pe
        return self.dropout(x)
    

class TimeEncodedLSTM(nn.Module):
    """
    LSTM with tAPE position encoding applied to inputs.
    
    The position encoding is applied BEFORE the LSTM to inject temporal
    position information into the input features.
    """
    
    def __init__(
        self, input_size: int, hidden_size: int, num_layers: int = 1, dropout: float = 0.1, 
              batch_first: bool = True, max_len: int = 1024, 
              tape_dropout: float = 0.1, tape_scale: float = 1.0, use_tape: bool = True):
        """
        Args:
            input_size: Number of input features
            hidden_size: LSTM hidden size
            num_layers: Number of LSTM layers
            dropout: LSTM dropout (applied between layers if num_layers > 1)
            batch_first: If True, input shape is (batch, seq, feature)
            max_len: Maximum sequence length for tAPE
            tape_dropout: Dropout rate for tAPE
            tape_scale: Scale factor for tAPE encoding magnitude
            use_tape: Whether to use tAPE (can disable for ablation studies)
        """
        super().__init__()
        ...
        # Project input to hidden_size if needed (for tAPE compatibility)
        if input_size != hidden_size:
            self.input_proj = nn.Linear(input_size, hidden_size)
        else:
            self.input_proj = None
        
        # tAPE: Applied to hidden_size dimension
        if self.use_tape:

            self.tape = TemporalAwaretAPE(
                d_model=hidden_size,
                dropout=dropout,
                scale_factor=tape_scale
            )
        
        # LSTM: Takes hidden_size as input after projection + tAPE
        self.lstm = nn.LSTM(
    ...
    def forward(self, x: torch.Tensor, x_dates: torch.Tensor, hidden=None):
        """
        Args:
            x: Input tensor of shape (batch, seq_len, input_size) if batch_first=True
            hidden: Optional LSTM hidden state tuple (h_0, c_0)
        
        Returns:
            output: LSTM output of shape (batch, seq_len, hidden_size)
            hidden: Final hidden state tuple (h_n, c_n)
        """
        # Step 1: Project input to hidden_size if needed
        if self.input_proj is not None:
            x = self.input_proj(x)
        
        # Step 2: Apply tAPE before LSTM
        if self.use_tape:
            x = self.tape(x, x_dates)
...
class MultiFreqInputEmbedding(nn.Module):
    """
    Multi-resolution LSTM + attention fusion embedding with numerical stability improvements.
    Dynamically accepts any number of side-channel inputs via kwargs.
    """
    ...
    def __init__(self, config):
        ...
        # Channel configurations ([(patch_len, stride, num_features), ...])
        log_in_local_rank_0(f'Channel Configs Inputted: {config.channel_configs}')
        self.channel_configs = getattr(config, "channel_configs", [])[1:]   # First Channel is the Price Channel, we ignore here
        self.num_channels = len(self.channel_configs)

        # Main LSTM for the primary input with layer normalization
        self.main_lstm = nn.LSTM(
            input_size=1,     # This is the price series
            hidden_size=config.hidden_size,
            batch_first=True,
            dropout=0.1  # Add dropout for stability
        )
        
        # Layer normalization for main LSTM output
        self.main_ln = nn.LayerNorm(config.hidden_size)

        # Each channel gets its own LSTM with normalization
        self.channel_lstms = nn.ModuleList()
        self.channel_lns = nn.ModuleList()
        
        for cfg in self.channel_configs:
            
            lstm = TimeEncodedLSTM(
                  input_size=cfg[2],  # number of features in the channel
                  hidden_size=config.hidden_size, 
                  batch_first=True,
                  num_layers=2,
            ...
            self.channel_lstms.append(lstm)
            self.channel_lns.append(ln)

        # Projections for attention with proper initialization
        ...
        # Initialize projections with smaller values
        for proj in [self.query_proj] + list(self.key_proj) + list(self.value_proj):
        ...
        self.act_fn = nn.GELU()
        self.dropout = nn.Dropout(getattr(config, "hidden_dropout_prob", 0.3))
    ...
    def _check_and_fix_nans(self, tensor, name="tensor"):
        """Check for NaNs/Infs and replace with zeros"""
    ...
    def _safe_interpolate(self, tensor, target_length):
        """Safely interpolate tensor to target length with NaN checking"""
        # Check input
        tensor = self._check_and_fix_nans(tensor, "pre-interpolate")
        
        # Clamp values to prevent extreme values before interpolation
        ...
        result = interpolated.transpose(1, 2)
        
        # Check output
        result = self._check_and_fix_nans(result, "post-interpolate")
        
        return result

    def forward(self, main_input, **kwargs):
        """
        Args:
            main_input: Tensor (B, T, F_main) - price series
            **kwargs: dynamically provided side channels,
                      e.g., channel_1=(B, patch_len, F₁), channel_2=(B, patch_len, F₂), ...
        Returns:
            fused: Tensor (B, T, hidden_size)
        """
        # Convert to tensor and check for NaNs
        if isinstance(main_input, np.ndarray):
            main_input = torch.tensor(main_input, dtype=torch.float32)
        
        main_input = self._check_and_fix_nans(main_input, "main_input")
        
        # Ensure main_input is on the correct device
        if hasattr(self.main_lstm, 'weight_ih_l0'):
            device = self.main_lstm.weight_ih_l0.device
            main_input = main_input.to(device)
        
        B, T, _ = main_input.shape

        # Step 1: Encode main sequence with LSTM
        main_emb, _ = self.main_lstm(main_input)
        main_emb = self._check_and_fix_nans(main_emb, "main_lstm_output")
        
        # Apply layer norm and activation
        main_emb = self.main_ln(main_emb)
        main_emb = self.act_fn(main_emb)
        main_emb = self._check_and_fix_nans(main_emb, "main_emb_after_activation")

        # Step 2: Encode each side channel dynamically
        encoded_channels = []
        
        for i in range(0, len(self.channel_configs)):            
            ch_input, ch_input_dates = kwargs.get(f'channel_{i+1}'), kwargs.get(f'channel_{i+1}_dates')
            if i >= self.num_channels:
                raise ValueError(
                    f"Received more channels ({len(kwargs)}) than configured ({self.num_channels})."
                )

            # Convert to tensor and check for NaNs
            if isinstance(ch_input, np.ndarray):
                ch_input = torch.tensor(ch_input, dtype=torch.float32)
            if isinstance(ch_input_dates, np.ndarray):
                ch_input_dates = torch.tensor(ch_input_dates, dtype=torch.float32)
            
            ch_input = self._check_and_fix_nans(ch_input, f"channel_{i}_input")
            ch_input = ch_input.to(main_input.device)
            
            ch_input_dates = self._check_and_fix_nans(ch_input_dates, f"channel_{i}_input_dates")
            ch_input_dates = ch_input_dates.to(main_input.device)
                        
            # Encode with LSTM
            ch_emb, _ = self.channel_lstms[i](ch_input, ch_input_dates)
            ch_emb = self._check_and_fix_nans(ch_emb, f"channel_{i}_lstm_output")
            
            # Apply layer norm and activation
            ch_emb = self.channel_lns[i](ch_emb)
            ch_emb = self.act_fn(ch_emb)
            ch_emb = self._check_and_fix_nans(ch_emb, f"channel_{i}_after_activation")
            
            # Interpolate to match main sequence length
            ch_emb = self._safe_interpolate(ch_emb, T)
            encoded_channels.append(ch_emb)

        # If no channels, return main embedding only
        if len(encoded_channels) == 0:
            log_in_local_rank_0(f'Warning! No side channels provided, returning main embedding only.')
            fused = self.output_proj(main_emb)
            fused = self.dropout(fused)
            fused = self.output_ln(fused + main_emb)
            return fused

        # Step 3: Prepare attention keys and values
        queries = self.query_proj(main_emb)
        queries = self._check_and_fix_nans(queries, "queries")
        
        keys_list = []
        values_list = []
        
        for i in range(len(encoded_channels)):
            key = self.key_proj[i](encoded_channels[i])
            value = self.value_proj[i](encoded_channels[i])
            
            key = self._check_and_fix_nans(key, f"key_{i}")
            value = self._check_and_fix_nans(value, f"value_{i}")
        ...
        keys = self._check_and_fix_nans(keys, "concatenated_keys")
        values = self._check_and_fix_nans(values, "concatenated_values")

        # Step 4: Attention-based fusion with gradient clipping
        try:
            fused, attn_weights = self.attn(queries, keys, values)
            fused = self._check_and_fix_nans(fused, "attention_output")
        except RuntimeError as e:
            print(f"Attention error: {e}")
            # Fallback to simple mean fusion
            fused = torch.mean(torch.stack([main_emb] + encoded_channels), dim=0)
            fused = self._check_and_fix_nans(fused, "fallback_fusion")

        # Step 5: Output projection + residual
        fused = self.output_proj(fused)
        fused = self._check_and_fix_nans(fused, "output_proj")
        
        # Residual connection with layer norm
        fused = self.output_ln(fused + main_emb)
        fused = self.dropout(fused)
        fused = self._check_and_fix_nans(fused, "final_output")

        return fused


# Additional utility functions for the dataset loader
def add_gradient_clipping_hook(model, clip_value=1.0):
    """
    Add gradient clipping hooks to all parameters in the model.
    Call this after model initialization.
    """
    for name, param in model.named_parameters():
        if param.requires_grad:
            param.register_hook(
                lambda grad, clip_val=clip_value: torch.clamp(
                    grad, -clip_val, clip_val
                ) if grad is not None else grad
            )


def check_model_outputs(outputs, stage=""):
    """
    Check model outputs for NaN/Inf values during training
    """
    if isinstance(outputs, torch.Tensor):
        if torch.isnan(outputs).any():
            print(f"NaN detected in outputs at {stage}")
            return False
        if torch.isinf(outputs).any():
            print(f"Inf detected in outputs at {stage}")
            return False
    return True
...
# Copied from transformers.models.xllama.modeling_llama.LlamaRMSNorm with Llama->TimeMOE
...
class TimeMoeAttention(nn.Module):
    ...
    def __init__(self, config: TimeMoeConfig, layer_idx: Optional[int] = None):
        ...
        if (self.head_dim * self.num_heads) != self.hidden_size or (self.head_dim * self.num_key_value_heads) != self.hidden_size:
            raise ValueError(
                f"hidden_size must be divisible by num_heads (got `hidden_size`: {self.hidden_size}"
                f" and `num_heads`: {self.num_heads}), 'num_heads_key_value_heads': {self.num_key_value_heads}."
            )
...
class TimeMoeModel(TimeMoePreTrainedModel):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.embed_layer = MultiFreqInputEmbedding(config)
    ...
    def forward(
            ...
            **kwargs,  # 👈 allows flexible input (e.g. multiple frequency channels)
        ...
        if inputs_embeds is None:
            # Sanity Check in TimeMoEModel: 
            #print(f'Pre-Multi-Freq Input Embedding Checking of Kwargs: {len(kwargs)}, {list(kwargs.keys())}')
            
            # Pass dynamic kwargs to embed layer (e.g., multiple frequency channels)
            inputs_embeds = self.embed_layer(input_ids, **kwargs)
...
class TimeMoeOutputLayerSingular(nn.Module):
...
class TimeMoeForPrediction(TimeMoePreTrainedModel, TSGenerationMixin):
    ...
    def forward(
        ...
        #log_in_local_rank_0(f"Input IDs into TimeMOEModelforPrediction: {input_ids.shape}, Prediction_labels: {labels.shape}")
        #print(f'TimeMoEForPrediction Kwargs: {len(kwargs)}')
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        #     if k == "self":  # skip model instance
        #         continue
        #     if k == "kwargs":
        #         # Print the side-channel keys too
        ...
        if labels is not None:
            ...
            for lm_head, horizon_length in zip(self.lm_heads, self.config.horizon_lengths):
                ...
                #ne_loss = self.calc_ar_loss_first_feature(one_predictions, labels, loss_masks, horizon_length)
                ar_loss += one_loss
            ...
            if horizon_length > max_horizon_length:
                ...
                predictions = predictions[:, :, : 1 * max_horizon_length]
    ...
    def calc_ar_loss(self, predictions, labels, loss_masks, horizon_length):
        ...
        labels = labels[..., 0:1] # We only keep the first feature   #### Added line
        ...
        if not torch.isfinite(loss):
            ...
            raise ValueError("Stopping training due to NaN/Inf loss")
